...
def banner():
    print("""%s
            ...
            \___ \| | | | '_ \|  _| | '_ \| | | | '_ ` _ \ 
             ___) | |_| | |_) | |___| | | | |_| | | | | | |
            |____/ \__,_|_.__/|_____|_| |_|\__,_|_| |_| |_| %s%s
                    # Coded By Muhammet Can Şimşek #
...
def get_args():
    ...
    parser = argparse.ArgumentParser('subdomain.py', formatter_class=lambda prog:argparse.HelpFormatter(prog,max_help_position=40),
            epilog="Specify a custom insertion point with %% in the domain name, such as: subdomain.py -d dev-%%.example.org")
    
    #Allow a user specifys a list of target domains
    target = parser.add_mutually_exclusive_group(required=True)
    
    target.add_argument('-d', '--domain', help='Target domains(sperated by commas)', dest='domain', required=False)

    parser.add_argument('-v', '--verbose', action="store_true", help='Verbose mode', dest='verbose' ,required=False)
...
def subdomain_scan():
    ...
    print("Target Domain(s) : ", args.domain)
        ...
        try:
            ...
            if ip_value:
                ...
                if f"{subdoms}.{args.domain}" in subdomain_store:
                    msg1 = "Discovered Subdomain"
                    msg2 = f'{subdoms}.{args.domain}'
                    out.good(msg1, msg2)
                else:
                    pass
            ...
            if args.verbose:
                msg1 = "Invalid Subdomain"
                msg2 = f'{subdoms}.{args.domain}'
                out.verbose(msg1, msg2)
            else:
                pass
